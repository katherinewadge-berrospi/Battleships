...
def get_dimensions():
    ...
    while True:
        try:
            rows = int(input("Number of rows (5 to 10): "))
            cols = int(input("Number of columns (5 to 10): "))
            if 5 <= rows <= 10 and 5 <= cols <= 10:
                return rows, cols
            print("Invalid input. Rows & columns must be between 5 and 10.")
        except ValueError:
